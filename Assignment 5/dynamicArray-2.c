...
int _minIdx(DynamicArray* heap, int parent, compareFunction compare);
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightIdx < last){
      ...
      minIdx = _minIdx(heap, position, compare);

      if (compare(dyGet(heap, minIdx), dyGet(heap, position))== -1){
        dySwap(heap, minIdx, position);
        adjustHeap(heap, last, minIdx, compare);
      }
    } 
...
{
    assert(heap != NULL);
    int maxIdx = dySize(heap);

    int i;

    for(i = maxIdx / 2 - 1; i >=0; i--){
      adjustHeap(heap, maxIdx, i, compare);
    }
}

/********************************************************
 ** name: _minIdx
 ** Params: heap, parent node, compare function
